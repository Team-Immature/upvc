...
const ButtonWithPopup = () => {
  ...
  return (
    <div className="relative w-full flex justify-center">
      ...
      {showPopup && (
        <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 w-[90vw] max-w-lg sm:w-64 bg-[linear-gradient(135deg,_#f0f4f8,_#d9e2ec)] text-gray-800 p-4 rounded-lg shadow-lg border border-gray-200 z-50">
          <h4 className="font-semibold text-lg mb-2">Available Colors</h4>
          <div className="grid grid-cols-2 gap-4">
            {colors.map(({ name, color }) => (
              <div key={name} className="flex items-center gap-2">
                <div className="w-8 h-8 border rounded-sm" style={{ backgroundColor: color }}></div>
